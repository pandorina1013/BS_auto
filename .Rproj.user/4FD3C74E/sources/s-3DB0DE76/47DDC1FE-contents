  ...
  for(i in 1:NROW(colnames.df)){
    ...
    for(j in 1:NROW(dist.name)){
      input.df %>% 
        ...
        arrange(min_rank) %>% 
        write.csv(paste0(filedir ,"/choose_", chr.col, "-", dist.name[j,], "_rankby_", num.col, ".csv"))
      message(paste0(filedir ,"/choose_", chr.col, "-", dist.name[j,], "_rankby_", num.col, ".csv"))
    }
    input.df %>% 
      mutate_(min_rank=paste0("min_rank(desc(",num.col,"))")) %>% 
      arrange(min_rank) %>% 
      write.csv(paste0(filedir ,"/choose_all", "_rankby_", num.col, ".csv"), row.names = F)
  }
  message("...and all!")
}